...
rm -f data/services/xray/json/*.json
rm -f data/services/hiddify-core/json/*.json
...
sed -i \
    -e "s|/opt/$GITHUB_REPOSITORY/menu.sh||g" \
    -e "s|/opt/$GITHUB_REPOSITORY/scripts/menu.sh||g" \
    -e "s|cd /opt/$GITHUB_REPOSITORY/||g" \
