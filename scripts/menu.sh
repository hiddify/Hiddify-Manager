...
function hiddify_bashrc_has_autostart() {
    grep -qE '/opt/hiddify-(manager|config|server)/(scripts/)?menu\.sh|^[[:space:]]*hiddify[[:space:]]*$' ~/.bashrc 2>/dev/null
}

function hiddify_bashrc_clean() {
    [ -f ~/.bashrc ] || return 0
    sed -i \
        -e 's|/opt/hiddify-config/menu.sh||g' \
        -e 's|/opt/hiddify-server/menu.sh||g' \
        -e 's|/opt/hiddify-manager/menu.sh||g' \
        -e 's|/opt/hiddify-manager/scripts/menu.sh||g' \
        -e 's|cd /opt/hiddify-manager/||g' \
        -e '/^[[:space:]]*hiddify[[:space:]]*$/d' \
        ~/.bashrc
}

function hiddify_bashrc_enable() {
    hiddify_bashrc_clean
    echo "hiddify" >>~/.bashrc
}

if hiddify_bashrc_has_autostart; then
    hiddify_bashrc_enable
fi
...
function menu() {
    ...
    'log')
        ...
        while read -r line; do
            size=$(ls -lah data/log/system/"$line" | awk -F " " '{print $5}')
            W+=("$line" "$size")
        done < <(ls -1 data/log/system)
        LOG=$(whiptail --clear \
            --backtitle "$BACKTITLE" \
            --title "$TITLE" \
            --menu "$MENU" \
        ...
        echo -e "\033[0m"
        if [[ $LOG != "" ]]; then
            less -r -P"Press q to exit" +G "data/log/system/$LOG"
        fi
        NEED_KEY=0
        ;;
    "advanced")
        ...
        "disable")
            ...
            ;;
        "uninstall")
            bash /opt/hiddify-manager/scripts/uninstall.sh
            ;;
        "purge")
            bash /opt/hiddify-manager/scripts/uninstall.sh purge
            ;;
        "add_remote")
            bash scripts/common/add_remote_assistant.sh
            ;;
        "remove_remote")
            bash scripts/common/remove_remote_assistant.sh
            ;;
        "warp")
            (
                cd services/warp/
    ...
    "update")
        ...
        case $CHOICE in
        "default")
            bash /opt/hiddify-manager/scripts/update.sh
            ;;
        "release")
            bash /opt/hiddify-manager/scripts/update.sh release
            ;;
        "beta")
            bash /opt/hiddify-manager/scripts/update.sh beta
            ;;
        *) NEED_KEY=0 ;;
        esac
        export CURRENT_CONFIG_VERSION=$(get_installed_config_version)
    ...
    "admin")
        ...
        ;;
    "status")
        bash /opt/hiddify-manager/scripts/status.sh | less -r -P"Press q to exit" +G
        NEED_KEY=0
        ;;
    *)
        bash "/opt/hiddify-manager/scripts/${CHOICE}.sh"
        ;;
    esac
