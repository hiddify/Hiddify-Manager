...
FORCE_SUBLINK_ONLY_DOMAIN=${FORCE_SUBLINK_ONLY//;/ } # Replace ; with spaces
if [ ${#FORCE_SUBLINK_ONLY_DOMAIN} -gt 5 ]; then
  sed -i "s|PANEL_DOMAINS|$FORCE_SUBLINK_ONLY_DOMAIN|g" common.cfg
fi
...
backend reality_h2_$i
    mode tcp
    server xray abns@realityinh2_$i send-proxy-v2
