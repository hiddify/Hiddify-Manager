...
proxy-providers:
  all_proxies:
    type: http
    url: "https://proxyproviderip/{{data["BASE_PROXY_PATH"]}}/{{data["user_id"]}}/clash/{{data["meta_or_normal"]}}/proxies.yml?2"
    path: proxyproviderip/{{data["user_id"]}}-{{data["meta_or_normal"]}}-proxies2.yaml
    health-check:
      enable: true
      interval: 600
