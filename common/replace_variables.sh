...
for template_file in $(find . -name "*.template"); do
    ...
    sed "s|defaultuserguidsecret|$GUID_SECRET|g" $template_file >$out_file
    sed "s|defaultcloudprovider|$CLOUD_PROVIDER|g" $template_file >$out_file
    sed "s|defaultserverip|$IP|g" $template_file >$out_file
    sed "s|defaultserverhost|$ROOT_DOMAIN|g" $template_file >$out_file
